    ...
    for field in solution:
        ...
        if solution[field]['match'] == 'exact':
            if field in student_output:
                ...
                grades[field]['match'] = student_output[field][
                                             'value'].lower() in variants

            if not grades[field]['match']:  # add expected output if not matched
                grades[field]['expected'] = solution[field]['value']
                grades[field]['obtained'] = student_output[field][
                    'value'].lower() if field in student_output else ''

        elif solution[field]['match'] == 'ordered':
            expected_lines = solution[field]['value']
            obtained_lines = student_output[field][
                'value'] if field in student_output else []
...
def parse_output(output, task='resolution', correct_output=False):
    ...
    for line in output_lines:
        if line.startswith("[CONCLUSION]:"):
            ...
            if correct_output:
                subtask_output['CONCLUSION'][
                    'match'] = 'exact' if task == 'resolution' else 'ordered'

            subtask_output['CONCLUSION']['value'].append(
                ' '.join(line.strip().split()[1:]))
            if task == 'resolution':
                subtask_output['CONCLUSION']['value'] = \
                subtask_output['CONCLUSION']['value'][0]
...
def log_results(log_file, report, verbose=False):
    ...
    log_file.write("\n=== TOTAL RESULTS ===\n")
    log_file.write(
        f"{total_passed} / {total_tests} tests passed. ({total_passed * 100. / total_tests}%)\n")

    print( f"{total_passed} / {total_tests} tests passed. {total_passed * 100. / total_tests} percent")

    print()
    print("failed tests")

    for subtask in sorted(report['evaluation_results'],
                          reverse=True):  # print RESOLUTION first
        log_file.write(f"\n== {subtask.upper()} ==\n")
        ...
        if verbose:  # logged in a separate file for each student with detailed info about failed tests
            for i in test_instances:
                if i['test_passed']:
                    ...
                    if not i['execute']:
                        log_file.write(
                            f"Execution failed with error (process returned non-zero exit code):\n{i['output']}\n")
                    elif not i['timeout']:
                        log_file.write("Execution timed out.\n")
                    else:
                        mismatched_fields = [field for field in
                                             i['field_results'] if
                                             not i['field_results'][field][
                                                 'match']]

                        for mis_field in mismatched_fields:
                            log_file.write(
                                f"- Mismatch with field: {mis_field}\n")
                            log_file.write("-> Obtained output:\n")
                            if isinstance(
                                i['field_results'][mis_field]['obtained'],
                                list):
                                log_file.write('\n'.join(
                                    i['field_results'][mis_field]['obtained']))
                            else:
                            ...
                            log_file.write(f"\n-> Expected output:\n")
                            if isinstance(
                                i['field_results'][mis_field]['expected'],
                                list):
                                log_file.write('\n'.join(
                                    i['field_results'][mis_field]['expected']))
                            else:
